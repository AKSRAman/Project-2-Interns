    ...
    try {
        ...
        if (DBlen != 0) {
            ...
            if (duplicateLogolink) {
                return res.status(400).send({ status: false, msg: 'This logo link belongs to other college.' })
            }
        }
        if (isDeleted === true) {
...
const getDetails = async function (req, res) {
    try {
        ...
        const interns = await internModel.find({ collegeId: id, isDeleted: false }).select({ _id: 1, name: 1, mobile: 1, email: 1 });
        if (interns.length < 1) {
            return res.status(400).send({ status: true, data: collegeDetails, msg: "No student applied in this college for internship." });
        }
        let details = { name, fullName, logoLink, interns }
...
module.exports = { getDetails, registerCollege }